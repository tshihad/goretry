...
func retry(
	...
	go func() {
		...
		retryRoutineFunc(ctx, rChan, cRetry, fn)
	}()
	go func(apiChan chan continueStruct) {
		for i := 0; ; i++ {
			...
			case continueChan := <-rChan.continueChan:
				...
				// call retry function
				go retryRoutineFunc(ctx, rChan, cRetry, fn)
...
func Retry(ctx context.Context, fn RetryFunc) (interface{}, error) {
	c := &CustomRetry{}
	c.setDefaultValues()
...
// CustomRetry struct.
func (c *CustomRetry) RetryParallel(ctx context.Context, fn RetryFunc) {
	c.setDefaultValues()
	retry(ctx, fn, c)
}
